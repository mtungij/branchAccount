     ...
     public function disburse_loan(){
      // $position   = strtoupper($this->session->userdata('position_name'));
        ...
        foreach ($this->queries->get_pending_topup_withdrawals_for_disburse_list($blanch_id) as $topup_row) {
            ...
            $totals = $this->calculate_topup_totals($loan_data, $new_principal, $topup_row->day, $topup_row->session, $topup_row->rate);
...
public function withdraw_loan_topup($topup_id)
{
    ...
    return redirect('oficer/data_with_depost/' . $topup->customer_id);
}
...
$branch_name=$customer_data->blanch_name;
      ...
      //             exit();
        ...
        $this->sendsms($phone, $message);
        $this->notify_whatsapp_deposit($phone, $message, $whatsapp_receipt_path);
          }else{
